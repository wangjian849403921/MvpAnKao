...
import com.mvpankao.ui.adapter.QueryReportAdapter;
...
import static com.mvpankao.utils.Constants.REPORT_CODE;
...
public class QueryReportsActivity extends BaseActivity implements QueryResultView {
    @BindView(R.id.back)
    ImageView mBack;
    ...
    List<ReportBean.ObjectBean.ListBean> list = new ArrayList<>();
    @BindView(R.id.report_query)
    SearchView mReportQuery;
    private QueryReportAdapter mAdapter;
    ...
    protected int getLayout() {
        return R.layout.ac_reportquery;
    }
    ...
    private void initData() {
        ...
        mQueryResultPresenter.initReportData(mContext, mMyOkhttp, list);
        ...
        switch (event.getMsg()) {
            case REPORT_CODE:
                ...
                if (mAdapter != null) {
                    ...
                    mAdapter = new QueryReportAdapter(mContext, list);
                    mReportRecycleView.setAdapter(mAdapter);

                }
                mAdapter.setOnItemOnClick(new QueryReportAdapter.OnItemClickListener() {
                    @Override
                    public void OnClick(View v, int position) {

                        String path = NetUrl.DOCHeader + list.get(position).getReportUrl();
                        LogUtils.d(path + "");
    ...
    public void onSuccess() {
        hideProgress();
        EventBus.getDefault().post(new MyEvent(REPORT_CODE));
    }
