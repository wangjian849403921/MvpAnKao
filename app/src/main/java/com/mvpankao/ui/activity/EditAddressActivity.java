...
public class EditAddressActivity extends BaseActivity implements OnAddressSelectedListener, AddressSelector.OnDialogCloseListener,EditAddressView {
    ...
    Switch mSwitchBtn;
    String Province, City, Area, PhoneNum;
    int isDefault = 0;
    String phone;
    ...
    protected void initView() {
        ...
        final boolean isDefault = intent.getBooleanExtra("isDefault", false);
        mEtName.setText(UserName);
        mArea.setText(Province + City + Area);
        mAddressDetail.setText(AddressDetail);
...
//        }
        mSwitchBtn.setChecked(isDefault);
        if (isDefault) {
...
//                if (!TextUtils.isEmpty(contacts[1]) && contacts[1].length() > 6) {
//                    StringBuilder sb = new StringBuilder();
//                    for (int i = 0; i < contacts[1].length(); i++) {
//                        char c = contacts[1].charAt(i);
//                        if (i >= 3 && i <= 6) {
//                            sb.append('*');
//                        } else {
//                            sb.append(c);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.rl_back:
                ...
                new AlertDialog(mContext).builder().setTitle("编辑地址")
                        .setMsg("您确定退出编辑？")
