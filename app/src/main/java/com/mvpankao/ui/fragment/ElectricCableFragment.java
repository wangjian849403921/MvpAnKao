...
public class ElectricCableFragment extends BaseFragment implements HomeProductView {
    ...
    @BindView(R.id.View3)
    View mView3;
    HomeProductPresenter mHomeProductPresenter;

    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        mHomeProductPresenter =new HomeProductPresenter(this);
    }
    ...
    protected void initEventAndData() {
      ...
      mHomeProductPresenter.initData(getActivity(),mMyOkhttp,list,"1");
    ...
    public void MyEvent(MyEvent event) {
        switch (event.getMsg()) {
            case Constants.HomePageProduct_CODE2:
                ...
                if (!StringUtils.isEmpty(list.get(1).getProductIcon())) {
                    Glide.with(getActivity()).load(NetUrl.DOCHeader + list.get(1).getProductIcon()).apply(options).into(mProductImage2);
                }
                if (!StringUtils.isEmpty(list.get(2).getProductIcon())) {
                    Glide.with(getActivity()).load(NetUrl.DOCHeader + list.get(2).getProductIcon()).apply(options).into(mProductImage3);
                }
    ...
    public void onSuccess() {
        ...
        setVisible();
        EventBus.getDefault().post(new MyEvent(Constants.HomePageProduct_CODE2));
    }
